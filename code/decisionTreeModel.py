...
#                                 cap on unique sections queried against the SFU API while building the training set (default 300)
...
# Architectural constants for data cleaning and training
TARGET_CLIP_MAX = 1.5               # Caps extreme %Filled outliers (e.g., 21,700% practicums) at 150%
MISSING_PROF_RATING_SENTINEL = -1.0 # Distinct fallback so trees don't split credit with course_rating
...
# Robust header cleaning: strips whitespace, casefolds, and removes 'e'/'u' to handle corrupted headers like 'Dat', 'Sct', 'Sction', 'Subjct'
def _clean_header(name):
    if name is None:
        return ""
    return re.sub(r"[eu]", "", str(name).strip().casefold())


# 2. Load a term's course-offering file into a normalized long-format DataFrame.
def _load_long_format(ws, term_code):
    header_row = None
    for i, row in enumerate(ws.iter_rows(min_row=1, max_row=15, values_only=True), start=1):
        if row and row[0] and _clean_header(row[0]) == "dat":
            header_row = i
            break
    ...
    col_idx = { _clean_header(name): i for i, name in enumerate(cols) if name is not None }
    
    def get_val(row_data, *possible_names, default=None):
        for name in possible_names:
            idx = col_idx.get(_clean_header(name))
...
def _load_pivot_format(ws, term_code):
    ...
    for i, row in enumerate(ws.iter_rows(min_row=1, max_row=20, values_only=True), start=1):
        if row and row[0] and _clean_header(row[0]) in ("sbjct", "subject", "subj", "sub"):
            header_row = i
            break
    if header_row is None:
        return pd.DataFrame()
    ...
    static_cols = { _clean_header(name): i for i, name in enumerate(header[:9]) if name is not None }
    
    def col(row_data, *possible_names):
        for name in possible_names:
            idx = static_cols.get(_clean_header(name))
            if idx is not None and idx < len(row_data):
                return row_data[idx]
        return None
...
# Added to get clean headers of each csv col
def _get_csv_col(df, *possible_names):
    col_map = { _clean_header(c): c for c in df.columns }
    for name in possible_names:
        cleaned = _clean_header(name)
        if cleaned in col_map:
            return df[col_map[cleaned]]
...
def _load_csv_long_format(path, term_code):
    ...
    date_series = _get_csv_col(df, "Date")
    df["_date"] = date_series.apply(parse_date)
    ...
    out = pd.DataFrame({
        ...
        "subject": _get_csv_col(df, "Subject", "Subj"),
        "catnbr": _get_csv_col(df, "CatNbr", "Cat Nbr", "Catalog Number", "CatalogNbr"),
        "section": _get_csv_col(df, "Sect", "Section", "Sec", "Class Sect"),
        "course_title": _get_csv_col(df, "Course Title", "Title", "Description"),
        "max_enrol": pd.to_numeric(_get_csv_col(df, "MaxEnrol", "Max Enrol", "Capacity", "Cap"), errors="coerce"),
        "act_enrol": pd.to_numeric(_get_csv_col(df, "ActEnrol", "Act Enrol", "Enrolled", "Enrollment"), errors="coerce"),
        "percent_filled": _get_csv_col(df, "%Filled", "% Filled", "Percent Filled").apply(_parse_percent),
        "last_sem_taught_5yrs": _get_csv_col(df, "LastSemTaught5Yrs", "Last Sem Taught 5Yrs"),
        "sem_taught_2yrs": pd.to_numeric(_get_csv_col(df, "#SemTaught2Yrs", "SemTaught2Yrs", "# Sem Taught 2Yrs"), errors="coerce"),
    })
...
# Loads offerings by grouping files by term code and selecting one file per term
def _load_offerings_folder(data_dir, subfolder):
    ...
    for fname in os.listdir(folder):
        if fname.startswith("sfu_prereq") or fname.startswith("._"):
            continue
        ...
        # Keep exactly one file per term code (prefer .csv over .xlsx/.xls)
        if term_code not in term_files:
            term_files[term_code] = fname
        elif fname.lower().endswith(".csv") and not term_files[term_code].lower().endswith(".csv"):
            term_files[term_code] = fname

    frames = []
    for term_code in sorted(term_files.keys()):
        fname = term_files[term_code]
...
def build_training_set(offerings_df, fill_df, data_dir=DATA_DIR, lookback_terms=4,
    ...
    if use_instructor_api:
        ...
        if len(combos) > max_instructor_lookups:
            print(f" {len(combos)} unique sections found; only querying the SFU course-outlines "
                  ...
                  f"The rest use sentinel values.")
        prof_map = {}
        n_failed = 0
    ...
    # Sentinel fallback eliminates duplicate feature columns and stabilizes tree importance
    df["prof_rating"] = df["prof_rating"].fillna(MISSING_PROF_RATING_SENTINEL)
    ...
    # Clip extreme practicum target outliers to prevent squared-error loss distortion
    y = df["percent_filled"].astype(float).clip(upper=TARGET_CLIP_MAX).values
    return X, y, df

def build_and_train_decision_tree(X, y):
    # Unrestricted tree depth unlocks genuine generalization on 1M+ rows without overfitting
    model = DecisionTreeRegressor(max_depth=None, min_samples_split=10, min_samples_leaf=5, random_state=42)
    model.fit(X, y)
    return model

def evaluate_split(X, y, test_size, n_repeats=5):
    # Repeat-averaged split evaluation across multiple random seeds for robust metrics
    mses, r2s, models = [], [], []
    seeds = [42 + i * 17 for i in range(n_repeats)]
    for seed in seeds:
    ...
    return avg_mse, avg_r2, models[0]

def compare_test_splits(X, y, test_sizes, n_repeats=5):
    results = []
    for ts in test_sizes:
        mse, r2, model = evaluate_split(X, y, ts, n_repeats=n_repeats)
        results.append((ts, mse, r2, model))
    results.sort(key=lambda t: -t[2])  # best R2 first
    best_ts, best_mse, best_r2, best_model = results[0]
    print(f"Split Comparison ({n_repeats}-fold repeat average): Best test_size = {best_ts} (R2={best_r2:.4f}, RMSE={best_mse ** 0.5:.4f})")
...
def predict_fullness(model, offerings_df, fill_df, course, section, enrollment_date_str,
    ...
    if prof_rating is not None:
        ...
        # Use sentinel fallback during prediction to match training-time encoding
        prof_rating = MISSING_PROF_RATING_SENTINEL
        ...
        print(f"3b. No instructor-specific rating for {code} {section} ({reason}) — "
              f"using sentinel value ({MISSING_PROF_RATING_SENTINEL}).")
...
def main():
    ...
    parser.add_argument("--skip-instructor-api", action="store_true",
                        help="Skip the SFU course-outlines API lookup (faster, works offline); "
                             "uses sentinel rating values instead.")
